    ...
    public class EventManager
    {
        ...
        public void addTrigger<T>(Trigger<T> trig)
        {
            if (!EventToTriggerManager.TryGetValue(typeof(T), out var manager))
            {
                ...
                EventToTriggerManager[typeof(T)] = manager;
            }
        ...
        {
            EventToTriggerManager[typeof(T)].emitEvent(e);
        }

        public void emitEventAfter<T>(T e, int priority)
        {
            EventToTriggerManager[typeof(T)].emitEventAfter(e, priority);
        }
        public void emitEventBefore<T>(T e, int priority)
        {
            EventToTriggerManager[typeof(T)].emitEventBefore(e, priority);
        }
    }
